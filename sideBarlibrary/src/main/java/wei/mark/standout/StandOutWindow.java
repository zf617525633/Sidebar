	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		if (intent != null) {
			...
			if (ACTION_SHOW.equals(action) || ACTION_RESTORE.equals(action)) {
			    if(getWindow(id)!=null&&getWindow(id).isShown()){
...
//		mNotificationManager.cancel(getClass().hashCode() + id);
     ...
     * @return
     */
    public boolean onTouchHandleMoveIgnoreX(int id, Window window, View view,
            MotionEvent event, int margin) {
        StandOutLayoutParams params = window.getLayoutParams();
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (window.touchInfo.moving
                    ...
                    if (Utils.isSet(window.flags,
                        ...
                        if(params.y < margin)
                            params.y = margin;
                        if(params.y > display.getHeight()- margin*4)
                            params.y = display.getHeight()- margin*4;
                        window.edit().setPosition(params.x, params.y).commit();
                    }
                }
                break;
            case MotionEvent.ACTION_UP:
                ...
                if (event.getPointerCount() == 1) {
                    ...
                    // bring to front on tap
                    boolean tap = Math.abs(totalDeltaY) < params.threshold;
                    if (tap
                            && Utils.isSet(
                                    window.flags,
                                    StandOutFlags.FLAG_WINDOW_BRING_TO_FRONT_ON_TAP)) {
